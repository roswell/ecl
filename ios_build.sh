...
config_options="${config_options} --with-cross-config=$source_dir/cross_config_ios"

build_dir="$source_dir/build_ios/"
install_prefix="$source_dir/archive_ios/"
...
select_ios()
{
  ...
  export CFLAGS="-m32 -arch $arch -miphoneos-version-min=$iphoneos_version_min -isysroot $ios_sdk_dir"
  ...
  export LD="ld"
  export LDFLAGS="-arch $arch -pipe -std=c99 -gdwarf-2 -mthumb -isysroot $ios_sdk_dir"
