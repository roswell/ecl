...
/* cach1.h -- thread-local cache for generic function dispatch. This cache is
...
ecl_search_cache(ecl_cache_ptr cache, uint64_t hash, cl_object key)
{
...
static inline void
upsert_cache(ecl_cache_ptr cache, uint64_t hash, cl_object key, cl_object val)
{
  ecl_internal_error("upsert: not implemented yet.");
}

static inline void
delete_cache(ecl_cache_ptr cache, uint64_t hash, cl_object key)
{
  ...
  *new_cache = *cache;
  ...
  for(int i = 0; i<size; i++) {
    for(int idx = 1; idx<16; idx++) {
      ...
      cl_object keys[] = {key};
      uint64_t hash = vector_hash_key1(keys);
      insert_cache(new_cache, hash, key, val);
    }
  }
  return new_cache;
}
                 ...
                 cl_object key, cl_object value)
{
  ...
  insert_cache(cache, hash, key, value);
