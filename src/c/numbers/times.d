...
{
        CASE_FIXNUM_FIXNUM {
                return _ecl_fix_times_fix(ecl_fix(x), ecl_fix(y));
        }
        CASE_FIXNUM_BIGNUM {
                return _ecl_big_times_fix(y, ecl_fix(x));
        }
        CASE_FIXNUM_RATIO;
        CASE_BIGNUM_RATIO {
        ...
        CASE_BIGNUM_FIXNUM {
                return _ecl_big_times_fix(x, ecl_fix(y));
        }
        CASE_BIGNUM_BIGNUM {
...
ecl_times(cl_object x, cl_object y)
{
	...
	case t_singlefloat: {
		...
		switch (type_of(y)) {
		case t_fixnum:
			return ecl_make_singlefloat(fx * ecl_fix(y));
		case t_bignum:
		case t_ratio:
			return ecl_make_singlefloat(fx * ecl_to_double(y));
		case t_singlefloat:
			return ecl_make_singlefloat(fx * ecl_single_float(y));
		case t_doublefloat:
			return ecl_make_doublefloat(fx * ecl_double_float(y));
#ifdef ECL_LONG_FLOAT
		case t_longfloat:
	...
	case t_longfloat: {
		...
		switch (type_of(y)) {
		case t_fixnum:
			return ecl_make_longfloat(lx * ecl_fix(y));
		case t_bignum:
		case t_ratio:
			return ecl_make_longfloat(lx * ecl_to_double(y));
		case t_singlefloat:
			return ecl_make_longfloat(lx * ecl_single_float(y));
		case t_doublefloat:
			return ecl_make_longfloat(lx * ecl_double_float(y));
		case t_longfloat:
