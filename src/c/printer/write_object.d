...
_ecl_will_print_as_hash(cl_object x)
{
  ...
  cl_object circle_counter = ecl_symbol_value_nonnull(the_env, @'si::*circle-counter*');
  cl_object circle_stack = ecl_symbol_value_nonnull(the_env, @'si::*circle-stack*');
...
si_write_object_with_circle(cl_object x, cl_object stream, cl_object print_function)
{
  ...
  if (circle && !Null(x) && !ECL_FIXNUMP(x) && !ECL_CHARACTERP(x) &&
      (ecl_t_of(x) != t_symbol || (Null(x->symbol.hpack))))
    {
      ...
      circle_counter = ecl_symbol_value_nonnull(env, @'si::*circle-counter*');
