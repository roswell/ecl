...
static uint32_t
xxh32_small(uint32_t *vector, uint32_t length, uint32_t seed)
{
  uint32_t acc, rem, idx, lane;
  rem = length, idx = 0;
  /* Step 1. Initialize internal accumulator */
  acc = seed + PRIME32_5;
...
xxh32(uint32_t *vector, uint32_t length, uint32_t seed)
{
  uint32_t acc, rem, idx, lane;
  if (length<4)
    return xxh32_small(vector, length, seed);
  rem = length, idx = 0;
...
static uint64_t
xxh64_small(uint64_t *vector, uint64_t length, uint64_t seed)
{
  ...
  acc = acc + length;
  /* Step 5. Consume remaining input */
  /* There may be 0, 4, 8, 12, 16, 20, 24 or 28 remaining bytes. */
  for(; rem>=1; rem--, idx++) {
    lane = vector[idx];
...
{
  volatile uint64_t acc, rem, idx, lane;
  if (length<4)
    return xxh64_small(vector, length, seed);
  rem = length, idx = 0;
  /* Step 1. Initialize internal accumulators */
